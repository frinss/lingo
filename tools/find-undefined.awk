...
#/^__[A-Za-záéíóú0-9 <>\-&'\.()\/?,"!\+à\[\]]+__ :(nine|eight|seven|six|five)::hammer:/ {
/^__[A-Za-záéíóú0-9 <>\-&'\.()\/?,"!\+à\[\]]+__ [_A-Za-z_]* :(nine|eight)::hammer:/ {
	++acc_hammer_lines
	current_entry = $2
	if (verbose)
		printf "#%d: %s\n", acc_hammer_lines, current_entry;
	hammer[current_entry] = 1
}

!/^__[A-Za-záéíóú0-9 <>\-&'\.()\/?,"!\+à\[\]]+__/ && !/^\r/ {
	++acc_weird_lines
 	#if (debug)
...
END {
	n = length_of_array(hammer)
	printf 	"Summary:\n  acc_hammer_lines = %d\n  acc_empty_lines = %d\n  acc_weird_lines = %d\n  length('hammer') = %d\n",
		acc_hammer_lines, acc_empty_lines, acc_weird_lines, n;
	check(hammer, "hammer")
}
